...
간단한 포트폴리오 입니다.
- 이약저약 챗봇(졸업작품)<br>
  [여기](#1-이약저약_챗봇)를 눌러 이 문서의 `1. 이약저약_챗봇`으로 이동할 수 있습니다.
- 2023년 데이터청년캠퍼스 팀프로젝트(식중독 예측과 관리를 위한 데이터분석)<br>
  [여기](#2-여름철-식중독-예측,-관리를-위한-빅데이터분석)를 눌러 이 문서의 `2023년 데이터청년캠퍼스 팀프로젝트(식중독 예측과 관리를 위한 데이터분석)`으로 이동할 수 있습니다.
- gpt-dalle 그림그리기 app(개인프로젝트, open-ai와 api를 활용)<br>
  [여기](#3-gpt_dalle로-그림그리기-app)를 눌러 이 문서의 `3.gpt-dalle 그림그리기 app`으로 이동할 수 있습니다.
- 8조코드(서울자전거 따릉이 사용분석 팀프로젝트)<br>
  [여기](#4-8조-코드(자전거-데이터분석))를 눌러 이 문서의 `8조코드(서울자전거 따릉이 사용분석 팀프로젝트)`으로 이동할 수 있습니다.
- 감귤착과량분석(개인프로젝트)<br>
  [여기](#5-감귤-착과량-분석)를 눌러 이 문서의 `5.감귤 착과량 분석`으로 이동할 수 있습니다.
![](../header.png)
...
데이터는 대한약사회(https://www.kpanet.or.kr/information/search01.jsp)에서 수집하였습니다.(현재는 url이 바뀌어 앞의 url로 접속이 안됩니다. 바뀐url: https://www.kpanet.or.kr/medicineSearchPage.cm?menuCd=1002110000)
...
- 대한약사회에서 크롤링을 하기 위해 페이지 넘기기, 검색 후 idx 번호를 수집하는 함수제작 <br><br>
- 성분별로 약이름, 성분, 성상, 효능, 용법, 경고, 복약지도 를 수집한 뒤 한글과 영어를 분리 <br><br>
- 토큰화 후 tf-idf, LDA를 이용한 토픽모델링, 동시출현 단어쌍 생성 <br><br>
- 동시출현 단어쌍을 이용해 약의 성분, 질문내용, 예상답변 csv파일을 버트모델을 사용해 임베딩하여 벡터화 시킨 뒤 챗봇학습 <br><br>
- 유저가 입력한 질문을 벡터화 시킨 뒤 기존에 학습시킨 csv파일과의 코사인 유사도를 측정해 가장 유사도가 높은 답변을 출력 <br><br>
...
사진의 질문과 가장 유사한 답변을 출력해주는 것을 볼 수 있습니다. 사진속 채팅의 유사도는 84% 입니다.
<br><br>


#### 다시한다면?
- 증명된 약사가 답변을 해주는 사이트에서 학습용 데이터를 더 많이 수집한 후 챗봇 학습시키기 <br><br>
- 스트림릿을 이용해 실제 챗봇과 채팅하는 듯한 효과주기 등이 있습니다.
...
## 분석순서
- 데이터로드 및 전처리(식중독 환자수, 기온, 약물가격 데이터)
   - 데이터 열 이름 재지정 및 타입 확인
- 간단한 시각화(이상치, 추세그래프)
- 식중독 환자수 ARIMA 모델 적용, 미래예측
- 기온 ARIMA모델 적용
- 약물가격 ARIMA모델 적용, 미래예측
- 다변량분석(환자수, 기온, 약물가격) VAR모델 적용
...
## 결과
 ...
 식중독 ARIMA 모델 적용 및 예측<br>
   ...
   대략적인 예측이(여름이 다가올수록 환자수 증가) 가능했습니다.

 약물금액 ARIMA 모델 적용 및 예측<br>
   모델 적용 결과
...
- 피드백 및 정확한 결과를 위해 개선해야 하는 것
   - 다변량 분석을 할 만큼 열이 많지 않아 아쉽다.<br>
...
#### 느낀점<br>
- 생각보다 코드들이 바뀐게 있어서 부지런히 공부해야겠다고 생각했습니다.
...
- 1년중 이용자가 가장 많은 월을 시각화 했습니다. 9월에 이용자가 많은 것을 알 수 있습니다. 이러한 이유로 9월의 자전거 이용자 데이터를 이용해 전처리, 시각화를 진행했습니다. <br><br>
- 성별의 경우 카테고리가 6개가 있어 전처리가 필요했습니다. 또한, 대문자와 소문자를 대문자로 통일하고, 성별을 알 수 없는 값들을 -1로 지정해 주었습니다. 그 후 남자는 0, 여자는 1의 값을 부여했습니다. <br><br><br>
...
#### 다시한다면?<br>
- 자전거의 이동거리 및 운동량과 탄소량의 상관관계를 분석 후 월, 년 별의 탄소배출량의 차이 시각화
<br><br>
- 연도별 상위, 하위 이용건수 대여소 분석 등이 있습니다.
...
- null값이 있는지 확인한 후, 각 열들과 착과량의 상관관계를 봤습니다. 단순한 corr 함수로는 잘 보이지 않아 추가적으로 수고와 수관폭은 scatter그래프를 이용하여, 새순데이터는 heatmap을 그려주었습니다.<br><br> 그 결과 수고의 경우 200과 300사이, 수관폭의 경우 300과 400사이에 많은 데이터가 몰려있는 것을 볼 수 있었습니다. 새순의 경우 9월에는 음의 상관관계를 보여주고 11월에는 점차 양의 상관관계를 보여주는 것을 알 수 있었습니다. <br><br>
- 위의 탐색과정을 통해 원래의 데이터 그대로를 사용한 결과와 새순데이터만을 사용한 결과와의 비교분석을 진행하게 되었습니다.<br><br>
- 해당 주제에서 사용한 모델은 선형회귀, Ridge, lasso, 랜덤포레스트, XG부스트 입니다. <br><br>
...
#### 다시한다면?<br>
- 수고, 수관폭, 수관폭 평균을 이용한 착과량 분석 <br><br>
- 새순과 착과량 DF을 보면 300개 이상의 착과량을 보여주는 9월 새순의 데이터는 주로 2.8 ~ 3.8이하의 데이터인 것을 알 수 있습니다. 높은 착과량을 내는 최적의 새순 데이터가 무엇인지 알고 싶습니다.
